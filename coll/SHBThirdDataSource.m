...
@implementation SHBThirdDataSource
...
- (CGSize)collectionView:(UICollectionView *)collectionView sizeFittingSize:(CGSize)size forItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    AAPLCollectionViewCell *cell = nil;
    cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([SHBCell class]) forIndexPath:indexPath];
    cSize = [(SHBCell *)cell getSize:@"第三分区"];
...
- (void)loadContent {
    [self loadContentWithBlock:^(AAPLLoading *loading) {
        void(^handler)(NSArray *, NSError *) = ^(NSArray *data, NSError *error) {
            ...
            if (data.count) {
                [loading updateWithContent:^(SHBThirdDataSource *object) {
                    object.items = data;
                }];
            } else {
                [loading updateWithNoContent:^(SHBThirdDataSource *object) {
                    object.items = @[];
                }];
            }
        };
